...
static void
dump_tok (str seed, const str &in)
{
  strbuf b (seed);
  b << in;
  str hashval = b;
  ...
  //
  // Compute SHA1 ( SHA1(k), k)
  //
  //   Where k = seed . in
  //
  sha1ctx sha;
  u_int8_t h[sha1::hashsize];
  mstr out (sha1::hashsize);
  sha.update (hashval.cstr (), hashval.len ());
  sha.final (&h);
  sha.reset ();
  ...
  aout << "#define\t" << in << "\t dearmor64 (\"" << armor64 (out) << "\")\n";
}

static void
dump_toks ()
{
  mstr seed (sha1::hashsize);

  if (!ptxa_seed) {

    /*
     * XXX -- cannot read from terminal and also fork. bleh
     *
    if (do_rnd_kbd)
      rndkbd ("txarpcc requires random bytes -- you can alternatively\n"
	      "avoid this message with the SEED (foobarXXXX); txaRPC command");
    random_init ();
    rnd.getbytes (seed, sha1::hashsize);
    *
    */

    aout << "#error No SEED given; unsafe compile\n";
    warn << "No SEED given; refusing to compile without security\n";
    exit (1);

  } else {
    sha1_hash (seed, ptxa_seed, ptxa_seed.len ());
  }

  alltoks.traverse (wrap (dump_tok, str (seed)));
}
